...
static GstMeta *video_inference_transform_meta (GstBuffer * buffer_model,
    GstMeta * meta_model, GstBuffer * buffer_bypass);
    ...
    GstVideoInferencePad * pad)
{
  ...
  if (!pred_valid) {
    GST_WARNING_OBJECT (self,
        "Subclass prediction is not valid, dropping buffer...");
...
static gboolean
gst_inference_check_prediction_id (GNode * node, gpointer data)
{
  Prediction *root = (Prediction *) node->data;
...
{
  Prediction *root = meta->prediction;
  Prediction *predict = g_malloc (sizeof (Prediction));
  predict->id = id;
  predict->node = NULL;

  g_node_traverse (root->node, G_LEVEL_ORDER, G_TRAVERSE_ALL, -1,
      gst_inference_check_prediction_id, predict);

  if (!predict->node) {
    GST_LOG_OBJECT (self, "No prediction with ID %d found", id);
    g_free (predict);
    predict = NULL;
  }
    ...
    GstBuffer * buffer, GstVideoInferencePad * pad)
{
  ...
  meta_bypass = gst_buffer_get_meta (bypass_buffer,
  ...
  while (!model_empty) {
    ...
    if (NULL == model_buffer) {
      ...
      meta_bypass =
          video_inference_transform_meta (model_buffer, (GstMeta *) meta_model,
          bypass_buffer);

      /* TODO: Emit inference signal */
      /* g_signal_emit (self, gst_video_inference_signals[NEW_INFERENCE_SIGNAL], 0, */
      /*                     meta_model, &frame_model, meta_bypass, pbpass); */
